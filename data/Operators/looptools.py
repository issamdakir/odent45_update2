...
class ODENT_OT_LopptoolsRelax(Operator):
    bl_idname = "odent.looptools_relax"
    bl_label = "Relax"
    ...
    def execute(self, context):
        ...
        if not cached:
            cache_write(
                "Relax", object, bm, self.input, False, False, loops, derived, mapping
            )
        ...
        for iteration in range(int(self.iterations)):
            ...
            move = [
                relax_calculate_verts(
                    bm_mod, self.interpolation, tknots, knots, tpoints, points, splines
                )
            ]
...
def relax_calculate_verts(
    bm_mod, interpolation, tknots, knots, tpoints, points, splines
):
    change = []
    move = []
    for i in range(len(knots)):
...
def get_strokes(self, context):
    ...
    if (
        ...
        and not looptools.gstretch_guide == None
    ):
        try:
...
def cache_read(tool, object, bm, input_method, boundaries):
    ...
    modifiers = [
        ...
        if mod.show_viewport and mod.type == "MIRROR"
    ]
...
def cache_write(
    tool, object, bm, input_method, boundaries, single_loops, loops, derived, mapping
):
...
def face_edgekeys(face):
    return [
        tuple(sorted([edge.verts[0].index, edge.verts[1].index])) for edge in face.edges
    ]
...
def get_parallel_loops(bm_mod, loops):
    ...
    for loop in loops:
        edgeloop = [
            [sorted([loop[0][i], loop[0][i + 1]]) for i in range(len(loop[0]) - 1)],
            loop[1],
        ]
        if loop[1]:  # circular
...
def move_verts(object, bm, mapping, move, lock, influence):
    if lock:
        ...
        if custom:
            ...
            mat = (
                bpy.context.region_data.view_matrix.copy() @ object.matrix_world.copy()
            )
    ...
    for loop in move:
        for index, loc in loop:
            ...
            if influence < 0:
                ...
                new_loc = loc * (influence / 100) + bm.verts[index].co * (
                    (100 - influence) / 100
                )
...
class LoopToolsProps(PropertyGroup):
    ...
    bridge_mode: EnumProperty(
        ...
        items=(
            ...
            (
                "shortest",
                "Shortest edge",
                "Slower algorithm with " "better vertex matching",
            ),
        ),
    ...
    circle_custom_radius: BoolProperty(
        name="Radius", description="Force a custom radius", default=False
    )
    ...
    circle_lock_x: BoolProperty(
        name="Lock X", description="Lock editing of the x-coordinate", default=False
    )
    circle_lock_y: BoolProperty(
        name="Lock Y", description="Lock editing of the y-coordinate", default=False
    )
    circle_lock_z: BoolProperty(
        name="Lock Z", description="Lock editing of the z-coordinate", default=False
    )
    ...
    flatten_plane: EnumProperty(
        ...
        items=(
            ...
            (
                "view",
                "View",
                "Flatten on a plane perpendicular to the " "viewing angle",
            ),
        ),
    ...
    gstretch_conversion: EnumProperty(
        ...
        items=(
            (
                "distance",
                "Distance",
                "Set the distance between vertices " "of the converted stroke",
            ),
            (
                "limit_vertices",
                "Limit vertices",
    ...
    gstretch_conversion_distance: FloatProperty(
        name="Distance",
        description="Absolute distance between vertices along the converted " "stroke",
        default=0.1,
        min=0.000001,
        soft_min=0.01,
        soft_max=100,
    ...
    gstretch_method: EnumProperty(
        ...
        items=(
            (
                "project",
                "Project",
                "Project vertices onto the stroke, "
                "using vertex normals and connected edges",
            ),
            (
                "irregular",
                "Spread",
                "Distribute vertices along the full "
    ...
    gstretch_guide: PointerProperty(
        name="GPencil object", description="Set GPencil object", type=bpy.types.Object
    )

    # relax properties
    relax_input: EnumProperty(
        name="Input",
        items=(
            (
                "all",
...
# Registration :
#################################################################################################
classes = [ODENT_OT_LopptoolsRelax, LoopToolsProps]
